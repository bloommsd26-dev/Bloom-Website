  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-10">
      ...
      <div className="rounded-4xl border border-espresso/10 bg-white p-8 shadow-sm lg:col-span-2">
        <h3 className="font-heading text-xl font-bold text-espresso mb-6">
          Inbound Messages (Last 30 Days)
        </h3>
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={messagesByDate} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              <defs>
          ...
          {volunteersByStatus.map((entry, index) => (
            <div key={entry.status} className="flex items-center gap-2">
              <span
                ...
                style={{
                  backgroundColor: STATUS_COLORS[entry.status] || COLORS[index % COLORS.length],
                }}
              />
              <span className="text-[10px] font-bold uppercase tracking-widest text-espresso/60">
