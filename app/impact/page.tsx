        ...
        <Container>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {impactMetrics.map((metric, idx) => (
              <div
                key={idx}
                className="bg-white p-12 flex flex-col justify-between hover:bg-horchata/5 transition-all rounded-3xl shadow-sm hover:shadow-md border border-espresso/5"
              >
                <div className="mb-10">
                  <p className="font-heading text-[10px] font-bold uppercase tracking-widest text-espresso/30 mb-2">
                    Record 0{idx + 1}
                  </p>
                  <ImpactCounter number={metric.number} label="" suffix={metric.suffix} />
                </div>
                <div>
                  <h3 className="font-heading text-2xl font-bold mb-3 text-espresso">
                    {metric.label}
                  </h3>
          ...
          <div className="space-y-40">
            {stories.map((story, idx) => (
              <div key={idx} className="editorial-grid">
                ...
                >
                  <p className="text-sm font-bold text-cinnamon uppercase tracking-widest mb-6">
                    {story.meta}
                  </p>
                  <h3 className="font-heading text-3xl font-bold mb-6 italic text-espresso">
                    "{story.title}"
                  </h3>
